...
module and use statements in them. Options are:
  ...
  -O, --obj-template <template>
                 Template expression for the dependency object target
...
def write_deps(outf, filename, deps, mods, opts):
    ...
    filedir, filebase = os.path.split(filebase)
    outf.write("%s: %s\n" % (opts.obj_template.replace('\\1',filebase), " ".join(deps)))
    for mod in mods:
	outf.write("%s: %s\n"
		   % (mod, " ".join(deps)))
...
def process_args():
    try:
	opts, args = getopt.getopt(sys.argv[1:], "ho:d:D:m:M:O:v",
				   ["help", "output=", "dep-re=",
				    "dep-template=", "mod-re=",
				    "mod-template=","obj-template=",
				    "verbose"])
	...
	if o in ("-O", "--obj-template"):
	    myopts.obj_template = a
	if o in ("-v", "--verbose"):
	    myopts.verbose = True
	...
	sys.stderr.write("obj-template = %s\n" % myopts.obj_template)
    return (myopts, args)
