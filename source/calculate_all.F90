      ...
      if(myrank_row_hw.eq.0) then
        MPI_points_all: do mcount=1,MPIpts
          if (mcount.ne.1) then ! Receive all points except the first (that was calculated at myrank_row_hw)
            ...
            call MPI_Recv(vdc              ,3*Npl            ,MPI_DOUBLE_PRECISION,stat(MPI_SOURCE),4900,MPI_Comm_Row_hw,stat,ierr)
            call MPI_Recv(total_vdc        ,3                ,MPI_DOUBLE_PRECISION,stat(MPI_SOURCE),5000,MPI_Comm_Row_hw,stat,ierr)
        ...
        call MPI_Send(vdc              ,3*Npl            ,MPI_DOUBLE_PRECISION,0,4900,MPI_Comm_Row_hw,ierr)
        call MPI_Send(total_vdc        ,3                ,MPI_DOUBLE_PRECISION,0,5000,MPI_Comm_Row_hw,ierr)
