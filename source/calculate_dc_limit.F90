...
subroutine calculate_dc_limit()
  ...
  if(myrank_row.eq.0) then
    ...
    if(myrank_col.eq.0) then
      ! Storing rank 0 counter to recover later
      ...
      if(iw.eq.0) then
        close(911)
          write(outputunit,"('[calculate_dc_limit] Emergency ""stop"" file found! Stopping after step ',i0,'...')") count
          call system ('rm stop')
          write(outputunit,"('[calculate_dc_limit] (""stop"" file deleted!)')")
        call MPI_Abort(MPI_COMM_WORLD,errorcode,ierr)
      end if
      ! Recovering rank 0 counter
