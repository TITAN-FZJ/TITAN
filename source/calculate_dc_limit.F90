      ...
      if(myrank_col.eq.0) then
        ...
        MPI_points_dclimit: do mcount=1,MPIpts_hw
          if (mcount.ne.1) then ! Receive all points except the first (that was calculated at myrank_row_hw)
            ...
            call MPI_Recv(vdc              ,3*Npl            ,MPI_DOUBLE_PRECISION,stat(MPI_SOURCE),45000+mpitag2,MPI_Comm_Col,stat,ierr)
            call MPI_Recv(total_vdc        ,3                ,MPI_DOUBLE_PRECISION,stat(MPI_SOURCE),45100+mpitag2,MPI_Comm_Col,stat,ierr)
        ...
        call MPI_Send(vdc              ,3*Npl            ,MPI_DOUBLE_PRECISION,0,45000+mpitag2,MPI_Comm_Col,ierr)
        call MPI_Send(total_vdc        ,3                ,MPI_DOUBLE_PRECISION,0,45100+mpitag2,MPI_Comm_Col,ierr)
