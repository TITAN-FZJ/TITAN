...
!--------------------- begin MPI vars --------------------
	...
	integer :: ncountkl
	ncountkl=n0sc*dimsigmaNpl*4
!^^^^^^^^^^^^^^^^^^^^^ end MPI vars ^^^^^^^^^^^^^^^^^^^^^^

	allocate( ttFintiikl(n0sc1:n0sc2,dimsigmaNpl,4),LxttFintiikl(n0sc1:n0sc2,dimsigmaNpl,4),LyttFintiikl(n0sc1:n0sc2,dimsigmaNpl,4),LzttFintiikl(n0sc1:n0sc2,dimsigmaNpl,4), STAT = AllocateStatus )
...
! Starting to calculate energy integral
	if (myrank_row.eq.masterrank) then ! Process 0 calculates one point, receives results from others and send new tasks if necessary
		...
		do i=2,nepoints
			...
			call MPI_Recv(ttFintiikl  ,ncountkl,MPI_DOUBLE_COMPLEX,MPI_ANY_SOURCE,21212121+mpitag,MPIComm_Row,stat,ierr)
			call MPI_Recv(LxttFintiikl,ncountkl,MPI_DOUBLE_COMPLEX,stat(MPI_SOURCE),32323232+mpitag,MPIComm_Row,stat,ierr)
			call MPI_Recv(LyttFintiikl,ncountkl,MPI_DOUBLE_COMPLEX,stat(MPI_SOURCE),43434343+mpitag,MPIComm_Row,stat,ierr)
			call MPI_Recv(LzttFintiikl,ncountkl,MPI_DOUBLE_COMPLEX,stat(MPI_SOURCE),54545454+mpitag,MPIComm_Row,stat,ierr)
			if(lverbose) then
		...
		do
			...
			call MPI_Send(ttFintiikl  ,ncountkl,MPI_DOUBLE_COMPLEX,masterrank,21212121+mpitag,MPIComm_Row,ierr)
			call MPI_Send(LxttFintiikl,ncountkl,MPI_DOUBLE_COMPLEX,masterrank,32323232+mpitag,MPIComm_Row,ierr)
			call MPI_Send(LyttFintiikl,ncountkl,MPI_DOUBLE_COMPLEX,masterrank,43434343+mpitag,MPIComm_Row,ierr)
			call MPI_Send(LzttFintiikl,ncountkl,MPI_DOUBLE_COMPLEX,masterrank,54545454+mpitag,MPIComm_Row,ierr)
