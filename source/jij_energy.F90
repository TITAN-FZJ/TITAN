...
subroutine jij_energy(Jij)
  ...
  use mod_parameters,    only: nOrb2, Un, q, eta
  ...
  do i = 1, s%nAtoms
    ...
    do mu = 1, 3
      ...
      dBxc_dm(i,mu,:,:) = -0.5d0*Un(i)*(pauli_dorb(mu,:,:) - (paulievec(i,:,:)) * evec(mu,i))
      ...
      do nu=1,3
        ...
        d2Bxc_dm2(i,mu,nu,:,:) = 0.5d0*Un(i)*d2Bxc_dm2(i,mu,nu,:,:)/mabs(i)
      end do
    end do
  end do
