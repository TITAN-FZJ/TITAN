...
!-------------- Check if current is calculated for Vdc -----------------
  ...
  ! Calculating vector in-plane perpendicular to current direction
  versor_Eperp = cross_unit(versor_oop,dirEfieldvec)

  ! Storing neighbors for longitudinal and transverse currents
  sha_longitudinal = 0
  sha_transverse   = 0
  longitudinal_neighbors = 0
  transverse_neighbors = 0
  do i=1,n0
    if(dot_product(r0(i,:),dirEfieldvec).gt.1.d-8) then
      longitudinal_neighbors = longitudinal_neighbors+1
      sha_longitudinal(longitudinal_neighbors) = i
    end if
    if(dot_product(r0(i,:),versor_Eperp).gt.1.d-8) then
      transverse_neighbors = transverse_neighbors+1
      sha_transverse(transverse_neighbors) = i
    end if
  end do
  if(myrank.eq.0) then
    write(outputunit_loop,"('[lattice_definitions] Longitudinal neighbors: ',10(i0,2x))") (sha_longitudinal(i),i=1,longitudinal_neighbors)
    write(outputunit_loop,"('[lattice_definitions]   Transverse neighbors: ',10(i0,2x))") (sha_transverse(i),i=1,transverse_neighbors)
  end if
  return
