program DHE
  ...
  real(double),allocatable      :: xp(:),fvecp(:),errjac(:) ! delete
...
!     lwa=neq*(3*neq+13)/2
    ...
    allocate( mz(Npl),hdel(Npl),mp(Npl),hdelp(Npl),mm(Npl),hdelm(Npl),fvec(neq),jac(neq,neq),wa(lwa), STAT = AllocateStatus )
    if (AllocateStatus.ne.0) then
      if(myrank.eq.0) write(*,"('[main] Not enough memory for: mz,hdel,mp,hdelp,mm,hdelm,fvec,jac,wa')")
      call MPI_Abort(MPI_COMM_WORLD,errorcode,ierr)
    end if
    ...
    allocate( xp(neq),fvecp(neq),errjac(neq) ) ! delete
...
!------------------------ Begin self-consistency -----------------------
  ...
  ! (Try to read eps1 and mz if available - includes hdel, hdelp and hdelm calculations)
    ...
    case(1)
      ...
      if(selfcon) then
        sc_solu(1:Npl)       = eps1
        sc_solu(Npl+1:2*Npl)   = real(mp)
        sc_solu(2*Npl+1:3*Npl) = aimag(mp)
    ...
    case default !  If file doesn't exist
      ...
      mp = zero
      if(FIELD) then
        mp = cmplx(mz*sin(hwtheta)*cos(hwphi),mz*sin(hwtheta)*sin(hwphi),double)
        mz = mz*cos(hwtheta)
      end if
      sc_solu(1:Npl)         = eps1
      sc_solu(Npl+1:2*Npl)   = real(mp)
...
!     do i=1,Npl
!       mabs(i) = sqrt((real(mp(i))**2)+(aimag(mp(i))**2)+(mz(i)**2))
!       mp(i)   = cmplx(mabs(i)*sin(hwtheta)*cos(hwphi),mabs(i)*sin(hwtheta)*sin(hwphi),double)
!       mz(i)   = mz(i)*cos(hwtheta)
!     end do

!     ! Writing new eps1 and mz to file after self-consistency
!     if(myrank.eq.0) then
!       iflag = 1
!       call readwritesc(iflag,err)
!     end if
...
!       call c05zaf(4*Npl,4*Npl,sc_solu,fvec,jac,4*Npl,xp,fvecp,1,errjac)

!       call selfconsistencyjacnag(4*Npl,sc_solu,fvec,jac,4*Npl,1)
!       call selfconsistencyjacnag(4*Npl,sc_solu,fvec,jac,4*Npl,2)
!       call selfconsistencyjacnag(4*Npl,xp,fvecp,jac,4*Npl,1)

!       call c05zaf(4*Npl,4*Npl,sc_solu,fvec,jac,4*Npl,xp,fvecp,2,errjac)
!       if(myrank.eq.0) then
!         do i=1,4*Npl
!           write(*,*) i,errjac(i)
!         end do
...
!           diag(Npl+1:4*Npl) = 100.d0
    ...
    do i = 1,Npl
      mabs(i)   = sqrt((real(mp(i))**2)+(aimag(mp(i))**2)+(mz(i)**2))
      mtheta(i) = acos(mz(i)/sqrt(abs(mp(i))**2 + mz(i)**2))
      mphi(i)   = atan2(aimag(mp(i)),real(mp(i)))
      if(lGSL) then
    ...
    if(myrank.eq.0) then
      ...
      do i=1,Npl
        write(*,"(4x,'Mx (',i2.0,')=',f11.8,4x,'My (',i2.0,')=',f11.8,4x,'Mz (',i2.0,')=',f11.8)") i,real(mp(i)),i,aimag(mp(i)),i,mz(i)
    ...
    case (6)
      ...
      disturbances_energy_loop: do count=1+skip_steps,MPIsteps
        ...
        if(myrank_row.eq.0) then
          ...
          disturbances(2,:)   = disturbances(2,:)/zi
          sdmat = 0.5d0*sdmat
    ...
    case (8)
      ...
      if(myrank_row.eq.0) then
        if(myrank_col.eq.0) then
          ...
          ! Creating files and writing headers
          if(.not.laddresults) then
            if(.not.lnorenorm) call openclose_chi_files(0)
            call openclose_disturbances_files(0)
            call openclose_currents_files(0)
            call openclose_beff_files(0)
          end if
...
!-------------- Deallocating variables that depend on Npl --------------
    ...
    deallocate(mz,hdel,mp,hdelp,mm,hdelm,fvec,jac,wa)
