program SHE
  ...
  real(double),allocatable      :: ldosu(:,:),ldosd(:,:)
...
!-------------------- Writing parameters on screen ---------------------
  if(myrank.eq.0) then
    ...
    case("fcc100")
      ...
      write(*,"(8x,'theta =',e9.2,'pi')") theta/pi
      write(*,"(1x,'Electric field direction: ',$)")
      write_direction_E_field_fcc100: select case (i)
      case (1:8)   !    In plane neighbors:
        write(*,"('neighbor ',i0)") i
      case default
        write(*,"('Other',/,1x,' E = (',f6.3,',',f6.3,',',f6.3,')')") dirEfieldvec(1),dirEfieldvec(2),dirEfieldvec(3)
...
!--------------- Writing data to be calculated on screen ---------------
  if(myrank.eq.0) then
    ...
    case (2)
      ...
      write(*,"(1x,'Number of points to calculate: ',i0)") npt1
    case (3)
      write(*,"(1x,'LDOS as a function of energy')")
      write(*,"(9x,'emin =',e9.2)") emin
      write(*,"(9x,'emax =',e9.2)") emax
      write(*,"(1x,'Number of points to calculate: ',i0)") npt1
    case (4)
...
!-------------------------- Begin first test part ----------------------
  if((myrank.eq.0).and.(itype.eq.0)) then
    ...
    allocate( ldosu(1,9),ldosd(1,9), STAT = AllocateStatus )
    if (AllocateStatus.ne.0) then
      write(*,"('[main] Not enough memory for: ldosu,ldosd')")
      call MPI_Abort(MPI_COMM_WORLD,errorcode,ierr)
    end if
    ...
    test1_energy_loop2: do count=1,npt1
      ...
      call ldos(e,ldosu(1,:),ldosd(1,:))

      ! Writing on files
      write(varm,"('./results/ldosu_bulk_ncp=',I0,'_eta=',E8.1,'_Utype=',i0,'_hwx=',E8.1,'_hwy=',E8.1,'_hwz=',E8.1,'_nosc.dat')") ncp,eta,Utype,hwx,hwy,hwz
      open (unit=17+i, file=varm,status='unknown')
!       write(unit=17+i,fmt="(5(e16.9,2x))") (e-Ef)*ry2ev,ldosu(1,1)/ry2ev,sum(ldosu(1,2:4))/ry2ev,sum(ldosu(1,5:9))/ry2ev,sum(ldosu(1,:))/ry2ev
      write(unit=17+i,fmt="(5(e16.9,2x))") (e)*ry2ev,ldosu(1,1)/ry2ev,sum(ldosu(1,2:4))/ry2ev,sum(ldosu(1,5:9))/ry2ev,sum(ldosu(1,:))/ry2ev

      write(varm,"('./results/ldosd_bulk_ncp=',I0,'_eta=',E8.1,'_Utype=',i0,'_hwx=',E8.1,'_hwy=',E8.1,'_hwz=',E8.1,'_nosc.dat')") ncp,eta,Utype,hwx,hwy,hwz
      open (unit=57+i, file=varm,status='unknown')
!       write(unit=57+i,fmt="(5(e16.9,2x))") (e-Ef)*ry2ev,ldosd(1,1)/ry2ev,sum(ldosd(1,2:4))/ry2ev,sum(ldosd(1,5:9))/ry2ev,sum(ldosd(1,:))/ry2ev
      write(unit=57+i,fmt="(5(e16.9,2x))") (e)*ry2ev,ldosd(1,1)/ry2ev,sum(ldosd(1,2:4))/ry2ev,sum(ldosd(1,5:9))/ry2ev,sum(ldosd(1,:))/ry2ev
...
! (Try to read eps1 and mag if available - includes hdel, usplus and usminus calculations)
  ...
  if(selfcon) then
    ...
    self_consistency: do while ((mdif.gt.tol).or.(sdif.gt.tol))
      if(myrank.eq.0) write(*,"('|------------------- Iteration ',i0,' -------------------|')") i
...
!============================= MAIN PROGRAM ============================
  ...
  !----------------------------- Begin test part -----------------------
    if(myrank.eq.0) then
      allocate( ldosu(Npl,9),ldosd(Npl,9), STAT = AllocateStatus )
      if (AllocateStatus.ne.0) then
        write(*,"('[main] Not enough memory for: ldosu,ldosd')")
        call MPI_Abort(MPI_COMM_WORLD,errorcode,ierr)
      end if
...
!         call ldos(e,ldosu,ldosd)
!         do i=1,Npl
!           write(varm,"('./results/SOC=',L1,'/Npl=',I0,'/ldosT_Fe_layer',I0,'_magaxis=',A,'_ncp=',I0,'_eta=',E8.1,'_Utype=',i0,'_hwx=',E8.1,'_hwy=',E8.1,'_hwz=',E8.1,'_nosc.dat')") SOC,Npl,i,magaxis,ncp,eta,Utype,hwx,hwy,hwz
!           open (7+i, file=varm,status='unknown')
!           write(7+i,"(e16.9,2x,e16.9,2x,e16.9,2x,e16.9)") e,ldosuT(i),ldosdT(i)

!           write(varm,"('./results/SOC=',L1,'/Npl=',I0,'/ldos_Fe_layer',I0,'_magaxis=',A,'_ncp=',I0,'_eta=',E8.1,'_Utype=',i0,'_hwx=',E8.1,'_hwy=',E8.1,'_hwz=',E8.1,'_nosc.dat')") SOC,Npl,i,magaxis,ncp,eta,Utype,hwx,hwy,hwz
!           open (17+i, file=varm,status='unknown')
!           write(17+i,"(e16.9,2x,e16.9,2x,e16.9,2x,e16.9)") e,ldosu(i),ldosd(i)
!         end do
...
!           write(varm,"('./results/SOC=',L1,'/Npl=',I0,'/ldos_Fe_layer',I0,'_magaxis=',A,'_ncp=',I0,'_eta=',E8.1,'_Utype=',i0,'_hwx=',E8.1,'_hwy=',E8.1,'_hwz=',E8.1,'_nosc.dat')") SOC,Npl,i,magaxis,ncp,eta,Utype,hwx,hwy,hwz
!           open (7+i, file=varm,status='unknown')
!           write(7+i,"(e16.9,2x,e16.9,2x,e16.9,2x,e16.9)") e,ldosu(i),ldosd(i)
!         end do
  ...
  case (3)
    if(myrank.eq.0) then
      ...
      ldos_energy_loop: do count=1,npt1
        ...
        ! Writing on files
        plane_loop_ldos: do i=1,Npl
          write(unit=17+i,fmt="(5(e16.9,2x))") e,sum(ldosu(i,:)),ldosu(i,1),sum(ldosu(i,2:4)),sum(ldosu(i,5:9))
          write(unit=57+i,fmt="(5(e16.9,2x))") e,sum(ldosd(i,:)),ldosd(i,1),sum(ldosd(i,2:4)),sum(ldosd(i,5:9))
        end do plane_loop_ldos
...
!-----------------------------------------------------------------------
  case (5)
    ...
    if(myrank_row.eq.0) then
      ...
      deallocate(identt,Umatorb,temp,chiorb)
...
!-----------------------------------------------------------------------
  case (6)
    ...
    if(myrank_row.eq.0) then
      ...
      deallocate(identt,Umatorb,templd,temp,chiorb,tchiorbiikl)
...
!-------------- Deallocating variables that depend on Npl --------------
  ...
  deallocate(mmlayer,layertype,mmlayermag,lambda,npart0)
  select case (plnn)
  case(1)
    deallocate(t00,t01)
  case(2)
