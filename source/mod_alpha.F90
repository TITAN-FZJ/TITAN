...
contains
  ...
  subroutine create_alpha_files()
    ...
    do i=1, s%nAtoms
      do j = 1, size(filename)-6
         write(varm,"('./results/',a1,'SOC/',a,'/',a,'/',a,'_',i0,a,a,a,a,a,'.dat')") output%SOCchar,trim(output%Sites),trim(folder),trim(filename(j)),i,trim(output%Energy),trim(output%info),trim(output%BField),trim(output%EFieldBZ),trim(output%SOC),trim(output%suffix)
      ...
      write(varm,"('./results/',a1,'SOC/',a,'/',a,'/',a,'_',i0,a,a,a,a,a,'.dat')") output%SOCchar,trim(output%Sites),trim(folder),trim(filename(5)),i,trim(output%Energy),trim(output%info),trim(output%BField),trim(output%EFieldBZ),trim(output%SOC),trim(output%suffix)
      ...
      write(varm,"('./results/',a1,'SOC/',a,'/',a,'/',a,'_',i0,a,a,a,a,a,'.dat')") output%SOCchar,trim(output%Sites),trim(folder),trim(filename(6)),i,trim(output%Energy),trim(output%info),trim(output%BField),trim(output%EFieldBZ),trim(output%SOC),trim(output%suffix)
      open (unit=55+5*s%nAtoms+i, file=varm, status='replace', form='formatted')
      write(unit=55+5*s%nAtoms+i, fmt="('#     energy    ,  gammaM   ,  alpha_LS')")

      write(varm,"('./results/',a1,'SOC/',a,'/',a,'/',a,'_',i0,a,a,a,a,a,'.dat')") output%SOCchar,trim(output%Sites),trim(folder),trim(filename(7)),i,trim(output%Energy),trim(output%info),trim(output%BField),trim(output%EFieldBZ),trim(output%SOC),trim(output%suffix)
      open (unit=55+6*s%nAtoms+i, file=varm, status='replace', form='formatted')
      write(unit=55+6*s%nAtoms+i, fmt="('#     energy    ,  gammaM   ,  alpha_SL')")

      write(varm,"('./results/',a1,'SOC/',a,'/',a,'/',a,'_',i0,a,a,a,a,a,'.dat')") output%SOCchar,trim(output%Sites),trim(folder),trim(filename(8)),i,trim(output%Energy),trim(output%info),trim(output%BField),trim(output%EFieldBZ),trim(output%SOC),trim(output%suffix)
      open (unit=55+7*s%nAtoms+i, file=varm, status='replace', form='formatted')
      write(unit=55+7*s%nAtoms+i, fmt="('#     energy    ,  gammaM   ,  alpha_LL')")

      write(varm,"('./results/',a1,'SOC/',a,'/',a,'/',a,'_',i0,a,a,a,a,a,'.dat')") output%SOCchar,trim(output%Sites),trim(folder),trim(filename(9)),i,trim(output%Energy),trim(output%info),trim(output%BField),trim(output%EFieldBZ),trim(output%SOC),trim(output%suffix)
      open (unit=55+8*s%nAtoms+i, file=varm, status='replace', form='formatted')
      write(unit=55+8*s%nAtoms+i, fmt="('#     energy    ,  gammaM   ,  alpha_St')")

      write(varm,"('./results/',a1,'SOC/',a,'/',a,'/',a,'_',i0,a,a,a,a,a,'.dat')") output%SOCchar,trim(output%Sites),trim(folder),trim(filename(10)),i,trim(output%Energy),trim(output%info),trim(output%BField),trim(output%EFieldBZ),trim(output%SOC),trim(output%suffix)
  ...
  subroutine write_alpha(e)
    ...
    do i = 1, s%nAtoms
      ...
      write(55 +   s%nAtoms+i,"(36(es16.9,2x))") e, -1.d0 * aimag(acarthf(2,2))/aimag(acarthf(2,3)), e / (mabs(i) * aimag(acarthf(2,3))), -mabs(i)*aimag(acarthf(2,2)) / e, &
                                                ((real(acarthf(q,p)), aimag(acarthf(q,p)), q = 1, 4), p = 1, 4)
      write(55 + 2*s%nAtoms+i,"(36(es16.9,2x))") e, -1.d0 * aimag(acartinv(2,2))/aimag(acartinv(2,3)), e / (mabs(i) * aimag(acartinv(2,3))), -mabs(i)*aimag(acartinv(2,2)) / e, &
