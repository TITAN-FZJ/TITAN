      ...
      do i = 1,s%nAtoms
        do mu = 1,nOrb
          do n = 1,dimens
            ...
            expec_singlet(mu,i) = expec_singlet(mu,i) + 0.5_dp*s%Types(s%Basis(i)%Material)%lambda(mu)*tanh_n(n)*real( conjg(hk(isigmamu2n(i,1,mu)+hdimens,n))*hk(isigmamu2n(i,2,mu),n) )
          end do
        end do
      end do
