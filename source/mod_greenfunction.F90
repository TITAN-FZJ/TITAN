...
contains
  ...
  subroutine green(er,ei,s,kp,gf)
    ...
    ! dimHsc*dimHsc is the dimension of the array
    ! both 1s are for the strides to use on each array
    ...
    if(.not.lsupercond) then
      ! Put the slab Green function [A(nAtoms*18,nAtoms*18)] in the A(i,j,mu,nu) form
      do j = 1,s%nAtoms
        do i = 1,s%nAtoms
      ...
      ! Put the slab Green function [A(nAtoms*36,nAtoms*36)] in the A(i,j,mu,nu) form
      do j = 1,s%nAtoms
        do i = 1,s%nAtoms
