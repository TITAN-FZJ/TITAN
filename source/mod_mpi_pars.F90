module mod_mpi_pars
	...
	use MPI
	implicit none
	integer :: myrank,numprocs,errorcode,ierr,count,mcount,mpitag
...
!   !   ...
!   ! MPIpts-1
!   subroutine build_cartesian_grid_field()
!     use mod_parameters, only: pnt,hwa_npts,hwa_npt1,hwt_npts,hwt_npt1,hwp_npts,hwp_npt1,deltae,emin,emax
!     implicit none
!     integer :: total_npt1 = hwa_npt1*hwt_npt1*hwp_npt1
...
!           write(*,"('[main] ERROR: number of processes not commensurable with total energy integral points!')")
!           write(*,"('[main] Number of MPI processes: ',i0)") numprocs
!           write(*,"('[main] Number of points required: ',i0)") total_npt1
!           write(*,"('[main] Number of points in the energy integral: ',i0)") pnt
!         end if
!         stop
!       end if
!       MPIsteps = 1
!       if(total_npt1*pnt.lt.numprocs) then ! If the number of processors is larger than complete calculation
!         if(myrank.eq.0) then
!           write(*,"('[main] ******************************** WARNING: ********************************')")
!           write(*,"('[main]              Number of processes exceeds the total needed! ')")
!           write(*,"('[main]     Completing ',i0,' points with more ',i0,' points not to waste computing time. ')") total_npt1,MPIpts-total_npt1
!           write(*,"('[main] **************************************************************************')")
!         end if
!         total_npt1 = MPIpts
!       else if(total_npt1*pnt.gt.numprocs) then ! If the number of processors is smaller than complete calculation, check commensurability
!         MPIsteps = ceiling(dble(total_npt1)/dble(MPIpts))
!         if(mod(total_npt1,MPIpts).ne.0) then
!           if(myrank.eq.0) then
!             write(*,"('[main] ******************************** WARNING: ********************************')")
!             write(*,"('[main] Number of points to be calculated is not commensurable with processes used!')")
!             write(*,"('[main]     Completing ',i0,' points with more ',i0,' points not to waste computing time. ')") total_npt1,MPIsteps*MPIpts-total_npt1
!             write(*,"('[main] **************************************************************************')")
!           end if
!           total_npt1 = MPIsteps*MPIpts
!         end if
!       end if
!     end if
!     if(total_npt1.ne.1) then
!       npts = total_npt1-1
!     else
!       npts = total_npt1
!     end if
!     ! Calculating variations of field
!     deltae = (emax - emin)/npts      ! variation of energy between each point
!     MPIdelta = deltae*MPIpts         ! variation of energy between MPI steps
