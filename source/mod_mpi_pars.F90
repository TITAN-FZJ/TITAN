...
contains
  ...
  subroutine build_cartesian_grid()
    use mod_parameters, only: pnt, npt1, npts, deltae, emin, emax, outputunit
    implicit none
  ...
  subroutine build_cartesian_grid_field(elements_in_a_row)
    use mod_parameters, only: total_hw_npt1,outputunit
    implicit none
    logical :: lreorder,lperiodic(2),lrow(2),lcol(2)
    integer :: MPIdims(2)
    ...
    if(numprocs.le.elements_in_a_row) then  ! If number of processes is less than necessary for 1 energy integral
      ...
      MPIsteps_hw = 1
      if(total_hw_npt1*elements_in_a_row.lt.numprocs) then ! If the number of processors is larger (but commensurable) than complete calculation
        if(myrank.eq.0) then
          write(outputunit,"('[build_cartesian_grid_field] ************************************** ERROR: **************************************')")
        ...
        if(mod(total_hw_npt1,MPIpts_hw).ne.0) then
          if(myrank.eq.0) then
            write(outputunit,"('[build_cartesian_grid_field] ************************************** ERROR: **************************************')")
    ...
    ! Creating subarrays of rows and columns
    lrow = [.true.,.false.]
    call MPI_Cart_sub(MPIComm_Grid_hw,lrow,MPIComm_Col_hw,ierr) ! communicator inside a column (between rows)
    lcol = [.false.,.true.]
    call MPI_Cart_sub(MPIComm_Grid_hw,lcol,MPIComm_Row_hw,ierr) ! communicator inside a row (between columns)

    ! Obtaining process rank inside its row and column
    call MPI_Comm_rank(MPIComm_Row_hw,myrank_row_hw,ierr) ! Obtaining rank number inside its row
    call MPI_Comm_rank(MPIComm_Col_hw,myrank_col_hw,ierr) ! Obtaining rank number inside its column
