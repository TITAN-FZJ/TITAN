...
!     lwa=neq*(3*neq+13)/2
    lwa=neq*(neq+1)/2
    allocate( sc_solu(neq),diag(neq),qtf(neq),w(neq,4),fvec(neq),jac(neq,neq),wa(lwa) )
    ...
    if(lslatec) then
      if(lnojac) then
        call dnsqe(sc_equations,sc_jacobian,2,neq,sc_solu,fvec,tol,0,ifail,wa,lwa)
      else
        call dnsqe(sc_equations,sc_jacobian,1,neq,sc_solu,fvec,tol,0,ifail,wa,lwa)
      end if
      ifail = ifail-1
    else
      if(lnojac) then
!         call c05nbf(sc_equations,neq,sc_solu,fvec,tol,wa,lwa,ifail)
        ...
        mode = 1
        factor = 100.d0
        call c05ncf(sc_equations,neq,sc_solu,fvec,tol,maxfev,ml,mr,epsfcn,diag,mode,factor,0,nfev,jac,4*Npl,wa,lwa,qtf,w,ifail)
      else
...
!         diag(Npl+1:4*Npl) = 100.d0
        factor = 100.d0
        call c05pcf(sc_equations_and_jacobian,neq,sc_solu,fvec,jac,neq,tol,maxfev,diag,mode,factor,0,nfev,njev,wa,lwa,qtf,w,ifail)
      end if
    end if
...
!     Writing new results (mx, my, mz and eps1) and mz to file
  ...
  subroutine sc_equations_and_jacobian(N,x,fvec,selfconjac,ldfjac,iflag)
    use mod_constants
    use mod_parameters
    use mod_f90_kind
