...
module mod_system
  ...
  real(double) :: a0 !< Lattice Parameter

  real(double), dimension(3) :: a1 !< 1st Bravais Lattice Vector
  real(double), dimension(3) :: a2 !< 2nd Bravais Lattice Vector
  real(double), dimension(3) :: a3 !< 3rd Bravais Lattice Vector

  real(double), dimension(3) :: b1 !< 1st Reciprocal Lattice Vector
  real(double), dimension(3) :: b2 !< 2nd Reciprocal Lattice Vector
  real(double), dimension(3) :: b3 !< 3rd Reciprocal Lattice Vector

  real(double), dimension(3) :: pln_normal = 0.d0 !< Plane-normal vector (default = 0)
  real(double), dimension(3) :: pln_a1 = 0.d0 !< 1st In-plane Bravais Lattice Vector
  real(double), dimension(3) :: pln_a2 = 0.d0 !< 2nd In-plane Bravais Lattice Vector


  real(double), dimension(:,:), allocatable :: r_nn !< Next neighbor positions
  real(double), dimension(:,:), allocatable :: c_nn !< Directional cosines of neighbors
  integer,      dimension(:,:), allocatable :: l_nn !< 2D index list for neighbors (stage,plane)
  real(double), dimension(:,:), allocatable :: kbz  !< K-Points in first BZ
  real(double), dimension(:),   allocatable :: wkbz !< Weights of K-Points

  real(double), dimension(:), allocatable :: stages_list !< Next neighbor distances
  real(double), dimension(:), allocatable :: planes_list !< Plane distances

  integer, dimension(:), allocatable :: pln_cnt !< Elements in each plane

  integer :: npln    !< Number of nearest neighbors planes - to mount (tri-)diagonal matrices
  integer :: nstages !< Number of nearest neighbor stages
  integer :: ncount  !< Number of neighbors
...
!   print *, nkpt
!
!   deallocate(r_nn, c_nn, l_nn, stages_list, planes_list)
! end program test
